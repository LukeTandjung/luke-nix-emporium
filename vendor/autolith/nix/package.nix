...
let
  ...
  autolithSystem = pkgs.sbcl.buildASDFSystem {
    ...
    # The parent flake supplies this source through a nested path input. Compile
    # the unpacked writable tree instead of the immutable parent source path.
    buildPhase = ''
      runHook preBuild

      export CL_SOURCE_REGISTRY="$CL_SOURCE_REGISTRY:$PWD//"
      export ASDF_OUTPUT_TRANSLATIONS="$PWD:$PWD:/nix/store:/nix/store"
      ${pkgs.sbcl}/bin/sbcl --dynamic-space-size 3000 < "$buildScript"
...
pkgs.writeShellApplication {
  ...
  text = ''
    ...
    # Isolate the packaged ASDF registry from a parent Lisp environment. The
    # Nix SBCL wrapper installs the complete source and output registries.
